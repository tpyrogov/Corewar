...
static void	v_invalid_param(int error_code, t_v_validation *valid)
{
	...
	ft_printf("{red}Invalid parameter {eoc}");
	if (error_code == WRONG_PARAM_NUM)
		ft_printf("{red}count{eoc}");
	else
	{
		v_type_of_wrong_arg(valid);
		ft_printf("{red}%d type %s {eoc}", valid->param_index, valid->type);
	}
	ft_printf("{red}for instruction %s{eoc}", g_cmd[valid->cur_cmd].cmd);
}
...
static void	v_direct_label(t_v_validation *valid)
{
	...
	v_pass_whitespaces(valid);
	v_get_error_string(valid);
	ft_printf("{red}No such label %s while attempting to dereference {eoc}",
			valid->buff);
	ft_printf("{red}token [TOKEN][%03d:%03d] DIRECT_LABEL \"%s\"{eoc}",
...
void		v_invalid_instruct(t_v_validation *valid)
{
	...
	v_pass_whitespaces(valid);
	v_get_error_string(valid);
	ft_printf("{red}Invalid instruction at token [TOKEN][%3d:%3d]{eoc}",
...
void		v_put_validation_error(int error_code, t_v_validation *valid)
{
	if (error_code == FILE_ERR)
		ft_printf("{red}Can't read source file %s{eoc}", valid->file_name);
